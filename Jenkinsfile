  ...
  stages {
    stage('Checkout Git TAG') {
      steps {
        ...
        checkout([$class: 'GitSCM',
                  ...
                  gitTool: 'Default',
                  submoduleCfg: [],
                  userRemoteConfigs: [[url: 'https://github.com/eclipse-zenoh/zenoh-plugin-webserver.git']]
                ])
      }
    }
    ...
    stage('x86_64-pc-windows-gnu build') {
      ...
      steps {
        ...
        zip zenoh-plugin-webserver-${LABEL}-x86_64-pc-windows-gnu.zip --junk-paths target/x86_64-pc-windows-gnu/release/*.dll
        '''
      }
    }
    ...
    stage('i686-pc-windows-gnu build') {
      ...
      steps {
        ...
        zip zenoh-plugin-webserver-${LABEL}-i686-pc-windows-gnu.zip --junk-paths target/i686-pc-windows-gnu/release/*.dll
        '''
      }
    }
